...
namespace EventBusDemo.Common.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        {
            _logger = applicationBuilder.ApplicationServices.GetService<ILogger<BusSubscriber>>();
            _serviceProvider = applicationBuilder.ApplicationServices.GetService<IServiceProvider>();
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            CorrelationContext correlationContext, Func<Task> func)
        {
            var currenctRetry = 0;
            ...
            {
                var retryMessage = currenctRetry == 0 ? string.Empty : $"Retry: {currenctRetry}'.";
            ...
            where TEvent : IEvent
        {
            _busClient.SubscribeAsync<TEvent, CorrelationContext>(async (_event, correlationContext) =>
            {
                ...
                ctx => ctx.UseSubscribeConfiguration(cfg => 
                    cfg.FromDeclaredQueue(x=> x.WithName(GetQueueName<TEvent>(_namespace))))
            );
